...
PanelWindow {
    ...
    function performQrScan() {
        ...
        let cmd = `bash ${Caching.aetherDir}/scripts/screenshot.sh --geometry "${root.geometryString}" --scan-qr`;
    ...
    function executeCapture(openEditor, isRecord) {
        let cmd = `bash ${Caching.aetherDir}/scripts/screenshot.sh --geometry "${root.geometryString}"`;
        if (isRecord) {
