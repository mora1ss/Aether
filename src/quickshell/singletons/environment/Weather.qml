    ...
    Process {
        ...
        command: {
            ...
            if (root._forceFetchMode) {
                ...
                cmd = Caching.aetherDir + "/scripts/weather.sh --getdata --location '" + locEscaped + "' --unit '" + curUnit + "' && cat \"" + cacheFile + "\"";
            } else {
                cmd = Caching.aetherDir + "/scripts/weather.sh --json --location '" + locEscaped + "' --unit '" + curUnit + "'";
            }
            return ["bash", "-c", cmd];
        }
