...
Item {
    ...
    FileView {
        ...
        path: (typeof Caching !== "undefined" && Caching.stateDir ? Caching.stateDir : ((Quickshell.env("HOME") ?? "") + "/.local/state/aether")) + "/qs_matugen_colors.json"
        watchChanges: true
        onFileChanged: reload()
        onLoaded: {
            try {
    ...
    Process {
        ...
        running: false
        command: {
            let assetsPath = Caching.aetherDir ? (Caching.aetherDir + "/assets/themes") : "";
            let userPath = Caching.stateDir ? (Caching.stateDir + "/themes") : (Caching.home + "/.local/state/aether/themes");
    ...
    function installFont(filePath, fileName) {
        ...
        let userFontsPath = Caching.stateDir ? (Caching.stateDir + "/fonts") : (Caching.home + "/.local/state/aether/fonts");
        let escapeBash = function(str) { return String(str).replace(/(["\\$`])/g, '\\$1'); };
            ...
            Item {
                ...
                RowLayout {
                    ...
                    ClickButton {
                        ...
                        onClicked: {
                            Quickshell.execDetached(["bash", Caching.aetherDir + "/scripts/qs_manager.sh", "toggle", "wallpaper"]);
                        }
                    }
